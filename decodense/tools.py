...
def orbsym(mol, mo_coeff):
        ...
        """
        if isinstance(mo_coeff, jnp.ndarray) or isinstance(mo_coeff, np.ndarray):
            if mo_coeff.ndim == 2:
                try:
...
def write_rdm1(mol: gto.Mole, part: str, \
               ...
               suffix: str = '') -> None:
        """
        this function writes a 1-RDM as a jnp or cube (default) file
        """
        # assertion
        assert part == 'atoms', '`write_rdm1` function only implemented for `atoms` partitioning'
        assert fmt in ['cube', 'jnp'], 'fmt arg to `write_rdm1` must be `cube` or `jnp`'
